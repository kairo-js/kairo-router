...
var KairoCommandRegistry = class {
    ...
    try {
      this.nativeRegistry.registerCommand(
        ...
        (origin, ...args) => {
          ...
          console.log(
            `[kairo-router:cmd] native callback name=${def.name} active=${active} runWhenInactive=${runWhenInactive} delegatable=${delegatable} addonId=${addonId ?? "<none>"} kairoId=${kairoId ?? "<none>"} args=${args.length}`
          );
  ...
  setDelegatable(map, unavailableMessages) {
    ...
    console.log(
      `[kairo-router:cmd] setDelegatable entries=${[...this.delegatable.entries()].map(([name, value]) => `${name}:${value}`).join(",")}`
    );
  }
  setupRoutedListener(receive) {
    return receive((id, message) => {
      ...
      console.log(
        `[kairo-router:cmd] routed event received active=${active} kairoId=${myKairoId ?? "<none>"} bytes=${message.length}`
      );
      if (!active) return;
      ...
      if (!myKairoId || payload.targetKairoId !== myKairoId) {
        console.log(
          `[kairo-router:cmd] routed event ignored target=${payload.targetKairoId} self=${myKairoId ?? "<none>"}`
        );
        return;
      }
