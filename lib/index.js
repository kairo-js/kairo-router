...
var COMMAND_ROUTED_EVENT = "kairo:cmd-routed";
...
var CommandRoutedPayloadSchema = Type.Object(
  {
...
var validateCommandRoutedPayload = compile(CommandRoutedPayloadSchema);
...
function reconstructOrigin(serialized) {
  switch (serialized.sourceType) {
    case "Block": {
      ...
      return { sourceType: CustomCommandSource.Block, sourceBlock: block, sourceEntity: void 0, initiator: void 0 };
    }
    case "Entity": {
      ...
      return { sourceType: CustomCommandSource.Entity, sourceBlock: void 0, sourceEntity: entity, initiator: void 0 };
    }
    case "NPCDialogue": {
      const npc = getEntityById(serialized.npcEntityId);
      const initiator = world3.getAllPlayers().find((p) => p.id === serialized.initiatorId);
      return { sourceType: CustomCommandSource.NPCDialogue, sourceBlock: void 0, sourceEntity: npc, initiator };
    }
    case "Server":
    default:
      return { sourceType: CustomCommandSource.Server, sourceBlock: void 0, sourceEntity: void 0, initiator: void 0 };
  }
}
function serializeArg(arg) {
  ...
  if (typeof arg === "object") {
    ...
    if (typeof obj["id"] === "string" && typeof obj["typeId"] === "string") {
      return obj["typeId"] === "minecraft:player" ? { type: "player", id: obj["id"] } : { type: "entity", id: obj["id"] };
    }
...
function reconstructArg(arg) {
  ...
  if (obj["type"] === "player" && typeof obj["id"] === "string") {
    return world3.getAllPlayers().find((p) => p.id === obj["id"]);
  }
  if (obj["type"] === "entity" && typeof obj["id"] === "string") {
    try {
      return world3.getEntity(obj["id"]);
    } catch {
      return void 0;
    }
  }
...
var KairoCommandRegistry = class {
  constructor(nativeRegistry, isActive, send, getAddonId, getKairoId) {
    this.nativeRegistry = nativeRegistry;
    this.isActive = isActive;
    this.send = send;
    this.getAddonId = getAddonId;
    this.getKairoId = getKairoId;
  }
  sealed = false;
  declarations = /* @__PURE__ */ new Map();
  delegatable = /* @__PURE__ */ new Map();
  unavailableMessages = /* @__PURE__ */ new Map();
  registerCommand(def, handler, options) {
      ...
      this.nativeRegistry.registerCommand(
        ...
        (origin, ...args) => {
          ...
          if (!addonId || !this.send) {
            console.warn(
              `[kairo-router:cmd] cannot relay name=${def.name} addonId=${addonId ?? "<none>"} send=${!!this.send}`
            );
  ...
  setDelegatable(map, unavailableMessages) {
    this.delegatable = new Map(map);
    this.unavailableMessages = unavailableMessages ? new Map(unavailableMessages) : /* @__PURE__ */ new Map();
  }
  setupRoutedListener(receive) {
...
var KairoStartupBeforeEvent = class {
  ...
  events;
  constructor(ev, isActive, apiRegistry, eventRegistry, commandRegistry) {
...
var ApiManifestBuilder = class {
  build(apiRegistry, eventRegistry) {
...
var KairoInitEventId = /* @__PURE__ */ ((KairoInitEventId2) => {
  ...
  KairoInitEventId2["OrderPing"] = "kairo:order-ping";
  KairoInitEventId2["OrderPong"] = "kairo:order-pong";
  KairoInitEventId2["CommandManifestRequest"] = "kairo:cmd-manifest-req";
  KairoInitEventId2["CommandManifest"] = "kairo:cmd-manifest";
  KairoInitEventId2["CommandDelegatableUpdate"] = "kairo:cmd-delegatable";
...
function isDynamicPropertyTarget(value) {
  return typeof value === "object" && value !== null && typeof value.getDynamicProperty === "function" && typeof value.setDynamicProperty === "function";
}
var KairoRouter = class {
  ...
  commandRegistry;
  commandManifestListener;
  commandDelegatableListener;
  commandRoutedListener;
  ...
  async save(targetOrKey, keyOrValue, value) {
    ...
    const result = await this.apiCallSender.request("kairo-database", "save", { key, value: storedValue });
    this.assertDatabaseAvailable(result);
  }
  async load(targetOrKey, keyOrOptions) {
    if (isDynamicPropertyTarget(targetOrKey)) {
      this.assertRunnable();
      return this.loadDynamicProperty(targetOrKey, String(keyOrOptions));
    }
    const key = targetOrKey;
  ...
  async delete(targetOrKey, key) {
    if (isDynamicPropertyTarget(targetOrKey)) {
      this.assertRunnable();
      targetOrKey.setDynamicProperty(String(key), void 0);
      return;
    }
    this.assertDatabaseDependency();
    this.assertRunnable();
    if (this.standaloneMode) return;
    const result = await this.apiCallSender.request("kairo-database", "delete", { key: targetOrKey });
    this.assertDatabaseAvailable(result);
  }
  async has(targetOrKey, keyOrOptions) {
    if (isDynamicPropertyTarget(targetOrKey)) {
      this.assertRunnable();
      return targetOrKey.getDynamicProperty(String(keyOrOptions)) !== void 0;
    }
    const key = targetOrKey;
    const options = keyOrOptions;
    this.assertDatabaseDependency();
  ...
  saveDynamicProperty(target, key, value) {
    if (value === void 0) {
      target.setDynamicProperty(key, void 0);
      return;
    }
  ...
  loadDynamicProperty(target, key) {
  ...
  dispose() {
    ...
    this.commandManifestListener = void 0;
    this.commandDelegatableListener?.dispose();
    this.commandDelegatableListener = void 0;
    this.commandRoutedListener?.dispose();
    this.commandRoutedListener = void 0;
    this.eventRegistry.clearActiveScopedListeners();
  ...
  startRouterListener(standalone = false) {
    ...
    if (!standalone) {
      this.orderPingListener = runtime.receive((id, _message) => {
        if (id !== "kairo:order-ping" /* OrderPing */) return;
        this.orderPingListener?.dispose();
        this.orderPingListener = void 0;
        runtime.send("kairo:order-pong" /* OrderPong */, JSON.stringify({ kairoId: context.kairoId }));
      });
    }
